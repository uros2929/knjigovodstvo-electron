...
let randomCitat = Math.floor((Math.random() * 68)),
    ...
    divZaCitat = document.getElementById("citatiRandom");
function ispisivanjeCitata() {
    divZaCitat.innerHTML = citatIspis;
}
ispisivanjeCitata();

let ugasiCitat = document.getElementById("ugasiCitat");
ugasiCitat.addEventListener('click', (event) => {
    event.preventDefault();
    divZaCitat.style.display = "none";
})
ugasiCitat.addEventListener('dblclick', (event) => {
    event.preventDefault();
    divZaCitat.style.display = "block";
})
